...
- 원물 도매시세는 시세판(sisepan.com)의 장어 도매가 페이지(전국 도매시장 경매
  결과 집계)를 하루 1회 읽어 경매 건 단위로 누적합니다. 페이지 구조가 바뀌면
  실행 로그에 경고가 남습니다. KMI 수산물 수급정보(fishdata.kmi.re.kr)의
  OPEN-API 연동은 추후 보강 후보입니다.
